      ...
      addItemsButton.addEventListener("click", (e) => {
        ...
        for (let i = 0; i < inputQtyAdd.length; i++) {
          if (!(inputQtyAdd[i].value === "" || inputQtyAdd[i].value === "0")) {
            ...
            outputDiv.classList.add("out-list-item-" + i);
            selectedMaterials.append(outputDiv);

            let mat = document.querySelectorAll(".out-list-item-" + i);

            console.log(mat);
            // Output Quantity
            qtyOutput(inputQtyAdd[i].value);
            // Out Description
            outDescription(descriptionAdd[i].innerText);
            // Out Unit Weight
            outputUnitWeight();
            // Out Total Weight
            outTotalWeight(inputQtyAdd[i].value, unitWeightAdd[i].innerText);
          }
        }
